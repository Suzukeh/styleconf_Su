...
function updateHelpContent() {
    if (helpLanguage === "ja")
    {
        helpContent.textContent =
            "styleconf_Su ヘルプ\n\n"
            + "【1. まず編集を始める】\n"
            + "・Open (Ctrl+O) で編集対象（New）を開きます\n"
            + "・必要なら Load Repo style.conf でリポジトリ版を読み込みます\n\n"
            + "【2. 変更前と比較する】\n"
            + "・Open Old style.conf で比較元（Old）を開きます\n"
            + "・左の行番号をクリックすると、Old と Editor の対応箇所へ移動できます\n"
            + "・Old 側の行番号をクリックすると、New と Editor の対応箇所へ移動できます\n"
            + "・不要になった比較は Clear Old で解除します\n\n"
            + "【3. 値を調整する】\n"
            + "・Editor で値を直接編集します\n"
            + "・Color format で色の表示を HEX / RGB / HSL に切り替えられます\n"
            + "・Before を使うと項目単位、右側の Reset を使うと行単位で元に戻せます\n"
            + "・Undo/Redo (Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y) で操作を戻せます\n\n"
            + "・History から過去の編集時点を確認し、その状態へ移動できます\n\n"
            + "【4. New と Old をまとめる】\n"
            + "・Merge を開き、項目ごとに New / Old を選んで適用します\n"
            + "・All New / All Old で一括選択できます\n\n"
            + "【5. 保存する】\n"
            + "・Save (Ctrl+S) で差分を確認して保存します\n\n"
            + "補足: 中央の境界線はドラッグで幅調整できます";
    ...
    } else
    {
        helpContent.textContent =
            "styleconf_Su Help\n\n"
            + "[1] Start editing\n"
            + "- Open (Ctrl+O) to load your target file into New\n"
            + "- Use Load Repo style.conf if you want the repository version\n\n"
            + "[2] Compare with an older file\n"
            + "- Open Old style.conf to load a comparison source\n"
            + "- Click left line numbers to jump to matching Old + Editor items\n"
            + "- Click Old line numbers to jump to matching New + Editor items\n"
            + "- Use Clear Old when you no longer need comparison\n\n"
            + "[3] Adjust values\n"
            + "- Edit values directly in the Editor panel\n"
            + "- Switch Color format between HEX / RGB / HSL as needed\n"
            + "- Use Before for item-level restore, and right Reset for line-level restore\n"
            + "- Use Undo/Redo (Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y) to step back/forward\n\n"
            + "- Use History to review previous states and jump to one\n\n"
            + "[4] Merge New and Old\n"
            + "- Open Merge and choose New or Old per item\n"
            + "- Use All New / All Old for bulk selection\n\n"
            + "[5] Save\n"
            + "- Save (Ctrl+S) to review changes and export\n\n"
            + "Tip: Drag pane dividers to resize the layout";
        helpRepoLink.textContent = "Repository: https://github.com/Suzukeh/styleconf_Su";
    }
